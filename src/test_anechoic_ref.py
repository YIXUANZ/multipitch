...
class Model(object):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for i, (s1_clean_feat, s2_clean_feat, est_feat1, est_feat2, s1_f0_label, s2_f0_label) in enumerate(train_loader):
                ...
                with autocast():
                    ...
                    loss = self.DCCRN_PITCH_criterion(est_f0_1, est_f0_2, s1_f0_label, s2_f0_label, perm) # matched permutation
    ...
    def validate(self, dccrn_pitch, eval_loader):
        ...
        with torch.no_grad():
            ...
            for k, (s1_clean_feat, s2_clean_feat, est_feat1, est_feat2, s1_f0_label, s2_f0_label) in enumerate(eval_loader):
                ...
                est_feat = torch.cat((est_feat1, est_feat2), 1)
                # complex stft feature
                # s1_clean_stft_feat, s2_clean_stft_feat = self.stft(s1_clean_feat[:,0,:]), self.stft(s2_clean_feat[:,0,:])
                est_stft_feat1, est_stft_feat2 = self.stft(est_feat1[:,0,:]), self.stft(est_feat2[:,0,:])
    ...
    def test(self, args, model_srate=8000, model_input_size=929):
        ...
        # separation network
        # model_file = '/ocean/projects/cis210082p/yixuanz/exps/crossnet_sep_libri360mix_reverb_100k_anechoic_target_sisdr_mc_r2/crossnet_sep_libri360mix_reverb_100k_anechoic_target_sisdr_mc_r2_best.model'
        model_file = '/fs/ess/PAA0005/yixuanz/psc_exp/crossnet_sep_libri360mix_reverbnoisy_100k_anechoic_target_sisdr_mc/crossnet_sep_libri360mix_reverbnoisy_100k_anechoic_target_sisdr_mc_best.model'
        # model_file = '/ocean/projects/cis210082p/yixuanz/exps/crossnet_sep_grid2mix_60k_sisdr_mc_checkpoint_ctd_/crossnet_sep_grid2mix_60k_sisdr_mc_checkpoint_ctd__best.model'
        crossnet = CrossNet(
            dim_input=2,
            dim_output=4,
            num_layers=12,
        ...
        pit = PermutationInvariantTraining(scale_invariant_signal_distortion_ratio, eval_func="max").to(self.device)                
        ...
        with torch.no_grad():
            ...
            for k, (mix_feat, target_feat, s1_f0_label, s2_f0_label) in enumerate(test_loader):
                ...
                print(sum(Etotal))
                total_err_list.append(sum(Etotal))
