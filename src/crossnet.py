...
from torch.nn import Parameter, init, MultiheadAttention
...
class LayerNormalization4DCF(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.ndim == 4:
            ...
            raise ValueError("Expect x to have 4 dimensions, but got {}".format(x.ndim))
        # mu_ = x.mean(dim=stat_dim, keepdim=True)  # [B,1,T,1]
        # std_ = torch.sqrt(x.var(dim=stat_dim, unbiased=False, keepdim=True))  # [B,1,T,F]
...
class EffCrossFrameSelfAttentionModule(nn.Module):
    def __init__(self,
                 embed_dim: int = 64,
                 n_freqs: int = 129,
                 num_heads: int = 4,
...
class CrossNetLayer(nn.Module):
    ...
    def __init__(
        ...
        self.cross_mhsa = EffCrossFrameSelfAttentionModule(embed_dim=dim_hidden, num_heads=num_heads, n_freqs= num_freqs, batch_first=True)
    ...
    def _csa(self, x: Tensor, attn_mask: Optional[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        x, attn = self.cross_mhsa(x, average_attn_weights=False, attn_mask=attn_mask)
...
class CrossNet(nn.Module):
    ...
    def __init__(
        ...
        # print("Adding output lstm")
        # self.out_lstm = LSTMPositionalEncdoing(
        #                 emb_dim = dim_hidden,
        #                 emb_ks = 4,
        #                 emb_hs = 1,
        #                 n_freqs = num_freqs, 
        #                 hidden_channels = 64, # 192
        #                 approx_qk_dim=512,
        #                 activation="prelu",
        #                 eps=1e-5,
        #         )
...
if __name__ == '__main__':
    # CUDA_VISIBLE_DEVICES=7, python -m models.arch.CrossNet
    ...
    # ms = get_module_summary(crossnet_small, module_args=(x,))
